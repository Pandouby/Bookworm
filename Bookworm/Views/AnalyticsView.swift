...
struct AnalyticsView: View {
    ...
    var body: some View {
        VStack(alignment: .center) {
            Chart(chartEntries, id: \.genre) { entry in
            ...
            .padding()
            LazyVGrid(
                columns: [GridItem(.adaptive(minimum: 100, maximum: .infinity))],
                alignment: .leading
            ) {
                ...
                ForEach(chartEntries) { entry in
                    HStack(alignment: .top) {
                        Image(systemName: "circle.fill")
                            .foregroundStyle(colorForGenre(entry.genre))  // Apply color to the system image
